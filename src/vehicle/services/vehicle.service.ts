...
export class VehicleService extends BaseService<Vehicle> {
  ...
  async create(vehicleDTO: VehicleDTO): Promise<Vehicle> {
    ...
    if (car)
      throw new BadRequestException(
        `Already exists a vehicle with this license plate ${vehicleDTO.licensePlate}`
      );
  ...
  async findAllWithOptions(options: VehicleGetOptions) {
    ...
    if (options.availableToRent)
      vehicles = vehicles.filter(
        (v) => !v.rents.filter((r) => !r.deletedAt).length
      );
    if (options.availableToRent === false)
      vehicles = vehicles.filter(
        (v) => v.rents.filter((r) => !r.deletedAt).length
      );

    return this.getAll({ where: { id: In(vehicles.map((v) => v.id)) } });
  }
}
